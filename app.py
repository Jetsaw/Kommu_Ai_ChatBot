...
from web_scraper import scrape as scrape_site   # 🔹 auto car scraper import
...
# ----------------- Load Supported Cars -----------------
try:
    SUPPORTED_CARS = {"cars": scrape_site()}
    log.info(f"[AutoCar] ✅ Loaded {len(SUPPORTED_CARS['cars'])} cars from Kommu.ai site.")
except Exception as e:
    log.error(f"[AutoCar] ❌ Failed to load supported cars: {e}")
    SUPPORTED_CARS = {"cars": []}
...
def auto_refresh():
    """Daily refresh SOP, warranty, and supported cars."""
    global SUPPORTED_CARS
    try:
        log.info("[AutoRefresh] 🔁 Starting daily refresh...")
        cars = scrape_site()
        SUPPORTED_CARS = {"cars": cars}
